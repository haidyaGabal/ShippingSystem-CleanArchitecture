...
namespace WebApi.Controllers
{
    ...
    public class PaymentMethodController : ControllerBase
    {
        ...
        [HttpGet]
        public ActionResult<ApiResponse<List<PaymentMethodDTO>>> Get()
        {
            try {
                var data = _paymentMethod.GetAll();

                return Ok( ApiResponse<List<PaymentMethodDTO>>.SuccessResponse(data));
            }
            catch(DataAccessException daEx)
            {
                ///500 internal server error
                return StatusCode(500, ApiResponse<List<PaymentMethodDTO>>.FailResponse(
